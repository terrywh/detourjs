...
module.exports = function(router) {
	router.post("/task/progress", async (ctx) => {
		...
		// 若 task 被用户提前终止，可能会发生找不到的情况，忽略即可
		if(task) await task.push(stat);
	...
	router.get("/task/start", async (ctx) => {
		...
		// 使用此 cookie 促使 nginx 绑定后端服务器
		ctx.cookies.set("detourjs_task_id", task.id, {maxAge: 3600000});
		ctx.body = {
			"errno": 0,
			"errmsg": "",
			"data": task.id,
	...
	router.get("/task/poll", async (ctx) => {
		...
		else ctx.body = {"errno": 0, "errmsg": "", "data": Task.errorPoll};
	});
